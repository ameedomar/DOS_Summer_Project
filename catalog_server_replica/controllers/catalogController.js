...
const CATALOG_SERVER_REPLICA_HOST = 'http://localhost:3000/'
...
async function updateBookDB(book, requestBook, headers) {
    ...
    if(!(headers['is-replica-update'] === 'true')) {//If this isn't a normal update from the order server
                                                    // (which means a replica update request) then we don't update the
                                                    // other replica again (to prevent infinite looping)
    ...
    return new Promise((resolve, reject) => { // to be sure from response arrived and db update -
                                                     // should wait to made update/delete ...
...
async function invalidateFrontendServer(bookId, title) {
